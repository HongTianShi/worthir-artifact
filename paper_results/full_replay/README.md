...
FiQA-Compression260 是本仓库提供的完整原始路线重建参考实现。其余任务采用同一套
工作流和适配器接口，但仓库不承诺自动下载或分发它们的原始语料、索引与模型。

该流程位于发布 ledger 的上游：检查用户另行取得的资源，调用任务适配器，把完整
query--route outcomes 转为 WorthIR ledger，并验证结果。FiQA-Compression260 不需要
手工准备索引；任务适配器会下载公开语料和模型，并在本地构建所需表示。
