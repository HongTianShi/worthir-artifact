"""Organizer-only per-query and cost-preference analyses."""
...
def _load_task(
    ...
    if len(matrix) != expected or len(decisions) != expected:
        raise ScoreError("task, ledger, and action query counts must agree")
    ...
    if set(decision_ids) != set(matrix):
        raise ScoreError("action membership does not match the evaluator ledger")
    unknown = sorted(
        {row["selected_route_id"] for row in decisions} - set(route_ids)
    )
    if unknown:
        raise ScoreError(f"unregistered selected routes: {unknown}")
...
def per_query_analysis(
    ...
    """Return evaluator-only query rows for one frozen action vector."""
    ...
    if lam < 0:
        raise ScoreError("lambda must be nonnegative")
...
def sensitivity_analysis(
    ...
    """Evaluate a frozen action vector over a declared or supplied lambda grid."""

    contract, routes, matrix, policy_id, decisions = _load_task(
        task_dir, action_path
    )
    if not lambda_grid or any(value < 0 for value in lambda_grid):
        raise ScoreError("lambda grid must contain nonnegative values")
    rows: list[dict[str, Any]] = []
...
def budget_analysis(
    ...
    """Summarize per-query effectiveness attainable under hard cost ceilings."""

    contract, routes, matrix, policy_id, decisions = _load_task(
        task_dir, action_path
    )
    if not budget_grid or any(value < 0 for value in budget_grid):
        raise ScoreError("budget grid must contain nonnegative values")
    route_ids = [route["route_id"] for route in routes]
    rank = {route_id: index for index, route_id in enumerate(route_ids)}
    fixed_route = contract["development_selected_fixed_route"]
    selected_by_query = {
...
def fixed_route_points(task_dir: Path) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    """Return aggregate fixed-route points and their Pareto membership."""
