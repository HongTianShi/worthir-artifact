#!/usr/bin/env python3
"""Convert a human-readable route-choice CSV into a bound WorthIR action file."""
...
def convert_actions(task: Path, source: Path, output: Path, policy_id: str) -> Path:
    if not policy_id:
        raise ActionError("policy_id must be nonempty")
    task = task.resolve()
    ...
    try:
        with source.open("r", encoding="utf-8-sig", newline="") as stream:
            ...
            if reader.fieldnames != ["query_uid", "selected_route_id"]:
                raise ActionError(
                    "choice CSV columns must be exactly: query_uid, selected_route_id"
                )
            decisions = list(reader)
    except OSError as exc:
        raise ActionError(f"cannot read choice CSV: {exc}") from exc
    decision_ids = [row["query_uid"] for row in decisions]
    if len(decision_ids) != len(set(decision_ids)):
        raise ActionError("choice CSV contains duplicate query_uid values")
    if set(decision_ids) != query_ids:
        missing = sorted(query_ids - set(decision_ids))
        extra = sorted(set(decision_ids) - query_ids)
        raise ActionError(f"query membership mismatch; missing={missing}, extra={extra}")
    unknown = sorted({row["selected_route_id"] for row in decisions} - route_ids)
    if unknown:
        raise ActionError(f"unknown selected routes: {unknown}")
    payload = {
