#!/usr/bin/env python3
"""Draw a dependency-free SVG of fixed-route and policy effectiveness--cost points."""
...
def draw(task: Path, actions: Path | None, destination: Path) -> None:
    ...
    elements = [
        ...
        f'<text x="110" y="38" class="title">Effectiveness--cost Pareto curve: {escape(metadata["task_id"])}</text>',
        '<text x="110" y="64" class="sub">Descriptive; evaluator-only. Fixed routes are blue and the frozen routing policy is orange.</text>',
    ]
    ...
    elements.extend([
        ...
        '<text x="525" y="625" text-anchor="middle" class="label">Mean cumulative cost</text>',
        f'<text x="28" y="330" text-anchor="middle" class="label" transform="rotate(-90 28 330)">Mean {escape(metadata["metric"])}</text>',
    ])
    if len(pareto) > 1:
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, help="private SVG destination")
    args = parser.parse_args()
    try:
        destination = output_path(args.task_dir, args.output, "pareto.svg")
        if destination.suffix.lower() != ".svg":
            raise ValueError("plot output must end in .svg")
