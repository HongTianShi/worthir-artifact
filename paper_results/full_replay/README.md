...
该流程位于发布 ledger 的上游：检查用户另行取得的语料与模型，调用任务适配器，把完整 query--route outcomes 转为 WorthIR ledger，并验证结果。仓库不再分发这些外部材料。
...
下载数据前先读 [`RESOURCE_REQUIREMENTS.md`](RESOURCE_REQUIREMENTS.md)，再查看 [`CANONICAL_TREC.md`](CANONICAL_TREC.md)、[`FEVER.md`](FEVER.md)、[`MUSIQUE.md`](MUSIQUE.md) 或 [`STRUCTURED_AND_DIAGNOSTIC.md`](STRUCTURED_AND_DIAGNOSTIC.md)。
